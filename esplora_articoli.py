...
class ArticoliExplorer:
    ...
    def print_articles_list(self, articles):
        ...
        print(f"  Pagina {self.current_page + 1}/{total_pages} ({total} articoli totali)\n")
        print("-" * 70)
        ...
        for i, art in enumerate(articles, 1):
            ...
            print(f"  {i:2}. [{art['id_articolo']:5}] {data} | {argomento:<20} | {titolo}")
    ...
    def _clean_html_for_docx(self, html):
        ...
        unsupported_tags = ['INPUT', 'TEXTAREA', 'SELECT', 'FORM', 'SCRIPT', 'STYLE', 'IFRAME', 'OBJECT', 'EMBED']
    ...
    def _is_html_content(self, text):
        ...
        import re
        html_tags = re.findall(r'<(?:p|div|table|tr|td|h[1-6]|ul|ol|li|strong|em|b|i|a|span|font)[^>]*>', 
                               text, re.IGNORECASE)
    ...
    def export_article(self, article):
        ...
        safe_title = "".join(c for c in (article['titolo_articolo'] or "articolo") 
...
def check_dependencies():
    ...
    logging.info("\n📦 Verifica dipendenze:")
    logging.info(f"  - python-docx: {'✅ OK' if DOCX_AVAILABLE else '❌ Non installato'}")
    logging.info(f"  - htmldocx:    {'✅ OK' if HTMLDOCX_AVAILABLE else '⚠️  Non installato (export senza formattazione HTML)'}")
    logging.info(f"  - beautifulsoup4: {'✅ OK' if BS4_AVAILABLE else '⚠️  Non installato (anteprima base)'}")
...
def main():
    ...
    parser.add_argument('db', nargs='?', default='articoli.db', help='Percorso del database SQLite')
    parser.add_argument('--page-size', type=int, default=10, help='Numero di articoli per pagina')
