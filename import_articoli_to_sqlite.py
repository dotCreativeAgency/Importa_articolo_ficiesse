...
class ImportManager:
    ...
    def get_existing_article(self, article_id):
        ...
        self.cursor.execute("SELECT * FROM t_articoli WHERE id_articolo = ?", (article_id,))
    ...
    def get_db_stats(self):
        ...
        if count > 0:
            self.cursor.execute("SELECT MIN(id_articolo), MAX(id_articolo) FROM t_articoli")
    ...
    def show_progress(self, current, total, article):
        ...
        print(f"\r  [{bar}] {progress:5.1f}% | {current}/{total} | {title:<50}", end='', flush=True)
    
    def import_file(self, sql_file):
        """Importa i dati da un file SQL"""
        import_dir = get_import_dir()
        filepath = os.path.join(import_dir, sql_file) if not os.path.isabs(sql_file) else sql_file
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Importa file SQL t_articoli in SQLite')
    parser.add_argument('file', nargs='?', help='File SQL da importare (o lascia vuoto per menu)')
    parser.add_argument('--db', '-d', default='articoli.db', help='Nome del database SQLite')
    parser.add_argument('--non-interactive', '-n', action='store_true', help='Modalità non interattiva (sostituisci duplicati)')
    parser.add_argument('--dry-run', action='store_true', help='Simula l\'import senza modificare il DB')
    parser.add_argument('--skip-duplicates', action='store_true', help='Skip lista duplicati')
    ...
    sql_file = os.path.join(import_dir, sql_file_arg) if not os.path.isabs(sql_file_arg) else sql_file_arg
